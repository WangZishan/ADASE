...
### Variant to sample frequency
tmpfile <- gzfile('data/Statistic/VariantListSta/VariantListSta.txt.gz','r'); Var2Sample_MSBB <- read.table(tmpfile,header = T,sep = '\t',stringsAsFactors = F); close(tmpfile)
tmpfile <- gzfile('analysis/ROSMAP/data/Statistic/VariantListSta/VariantListSta.txt.gz','r'); Var2Sample_ROSMAP <- read.table(tmpfile,header = T,sep = '\t',stringsAsFactors = F); close(tmpfile)
...
for (i in 1:length(Regions)) {
  ...
  l2 <- data.frame(l2,refCount=NA,altCount=NA,Count=NA,Count_Increase=NA,Count_Decrease=NA,Freq=NA,Freq_Increase=NA,Freq_Decrease=NA,mapped_gene_name=NA,HGVSc=NA,HGVSp=NA,Label=F,Label_HGVS=NA,CountFromVarSta=NA,stringsAsFactors = F)
  ...
  for (k in 1:nrow(l2)) {
    ...
    if(Regions[i]=='DLPFC'){
      tmpind <- match(l2$HGVSg[k],Var2Sample_ROSMAP$HGVSg)
      l2$CountFromVarSta[k] <- Var2Sample_ROSMAP[tmpind,paste0('SigHGVSg_',l2$Status[k],'Sam')]
      l2$Freq[k] <- l2$Count[k]/Var2Sample_ROSMAP[tmpind,paste0('AllHGVSg_',l2$Status[k],'Sam')]
      l2$Freq_Decrease[k] <- l2$Count_Decrease[k]/Var2Sample_ROSMAP[tmpind,paste0('AllHGVSg_',l2$Status[k],'Sam')]
      l2$Freq_Increase[k] <- l2$Count_Increase[k]/Var2Sample_ROSMAP[tmpind,paste0('AllHGVSg_',l2$Status[k],'Sam')]
    }else{
      tmpind <- match(l2$HGVSg[k],Var2Sample_MSBB$HGVSg)
      l2$CountFromVarSta[k] <- Var2Sample_MSBB[tmpind,paste0('SigHGVSg_',l2$Status[k],'Sam_',Regions[i])]
      l2$Freq[k] <- l2$Count[k]/Var2Sample_MSBB[tmpind,paste0('AllHGVSg_',l2$Status[k],'Sam_',Regions[i])]
      l2$Freq_Decrease[k] <- l2$Count_Decrease[k]/Var2Sample_MSBB[tmpind,paste0('AllHGVSg_',l2$Status[k],'Sam_',Regions[i])]
      l2$Freq_Increase[k] <- l2$Count_Increase[k]/Var2Sample_MSBB[tmpind,paste0('AllHGVSg_',l2$Status[k],'Sam_',Regions[i])]
    }
    if(!is.na(l2$HGVSp[k])){
  ...
  l3 <- l2[l2$Label,]
  write.table(l2,file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'_All.txt')),row.names = F,col.names = F,sep = '\t',quote = F)
  write.table(l3[l3$Label,],file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'_Label.txt')),row.names = F,col.names = T,sep = '\t',quote = F)
  write.table(l3$HGVSg[l3$Label],file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'_LabelHGVSg.txt')),row.names = F,col.names = F,sep = '\t',quote = F)
  ...
  p <- p + geom_point(data = l3,aes(x=log10(refCount+1),y=log10(altCount+1),color=Status,size=Freq),alpha=0.8,stroke=0) + scale_size(limits = c(0, 1))
  ...
  # p
  ggsave(p,filename=file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'.pdf')),h=5.5,w=5,useDingbat=F)
  ...
  p <- p + theme_bw() + theme(axis.text = element_text(size = 15),axis.title = element_text(size = 18),plot.title = element_text(hjust = 0.5, face = 'bold'),
                              ...
                              panel.grid = element_blank())
  p
  ggsave(file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'_1.pdf')),h=5.5,w=5,useDingbat=F)
  ...
  ggsave(p1,filename = file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'-AllGenes.pdf')),h=10,w=20,useDingbat=F)
  ...
  ggsave(p1,filename = file.path('analysis/ASE_individual_analysis/AD_Gene_Overlap_With_Ours/GeneOverlap/Freq2/',paste0(Regions[i],'-AllGenes_1.pdf')),h=10,w=20,useDingbat=F)
  
  cat(max(l3$Freq)); cat('   '); 
  cat(max(unlist(l3[,c('Freq_Increase','Freq_Decrease')]))); cat('   '); 
  cat(max(unlist(l3[,c('Count_Increase','Count_Decrease')]))); cat('  ');
  cat(nrow(l2)); cat('  ');cat(sum(l2$Count==l2$CountFromVarSta)); cat('\n')
}# i
